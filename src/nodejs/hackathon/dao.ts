	...
	FROM (
		...
		FROM (SELECT
				CONCAT_WS(' AND ', 
					NULLIF(
						SUBSTRING_INDEX(
							GROUP_CONCAT(t1.householdName SEPARATOR ', '), 
							', ', 
							length( GROUP_CONCAT(t1.householdName SEPARATOR ', ')) 
								- length(replace( GROUP_CONCAT(t1.householdName SEPARATOR ', '),', ',' '))
							), ''), 
					SUBSTRING_INDEX(GROUP_CONCAT(t1.householdName SEPARATOR ', '), ', ', -1)) 
					AS householdGivenNames,
				t1.plid,
				...
				FROM ((SELECT pl.address aid,
							...
							pl.address,
							pl.ordering,
							IF (hp.status = 'Deactivate', 
								null, 
								NULLIF( 
									CONCAT_WS(
										', ', 
										TRIM(IFNULL(hp.primary_given_name, h.primary_given_name)), 
										IF (TRIM(IFNULL(hp.secondary_given_name, h.secondary_given_name) = ''), 
											NULL, 
											TRIM(IFNULL(hp.secondary_given_name, h.secondary_given_name)) 
										)
									), 
								'')
							) householdName
						...
						LEFT JOIN households_public hp ON hp.household=h.id 
						WHERE pl.member_id=? AND pl.daily>0 )
					UNION ALL 
						(SELECT
								pl.address aid,
								pl.id as plid,
								...
								NULLIF( CONCAT_WS(
									', ', 
									TRIM(hp.primary_given_name), 
									IF (TRIM(hp.secondary_given_name)='',NULL,TRIM(hp.secondary_given_name)) ) , '' 
								) householdName
							FROM prayer_lists pl
							INNER JOIN households_public hp ON hp.address=pl.address 
								AND hp.household IS NULL 
								AND (hp.status != 'Deactivate') 
							WHERE pl.member_id=? AND pl.daily>0 )) t1
							GROUP BY t1.plid
						) t3
